...
const moreThan10Words = sentencesExercise5.filter(
  (sentence) => sentence.split(' ').length > 10
);
console.log(moreThan10Words);
...
const outOfStock = productsExercise6.filter(
...
const divisibleByThree = numbersExercise7.filter((number) => number % 3 === 0);
...
const passingGrade = ['A', 'A-', 'A+', 'B', 'B+', 'B-', 'C', 'C+'];
const studentsWithPassingGrade = studentsExercise9.filter((student) =>
  passingGrade.includes(student.grade)
);

console.log(studentsWithPassingGrade);
...
const usersStartWithJ = usersExercise10.filter((user) =>
  user.name.startsWith('J')
);
console.log(usersStartWithJ);
