...
const longSentences = sentencesExercise5.filter(
...
const studentsWithGradeLowerThanC = studentsExercise9.filter(
...
const usersWithNamesStartingWithJ = usersExercise10.filter((user) =>
  user.name.startsWith('J')
);
console.log('Users with names starting with J:', usersWithNamesStartingWithJ);
