...
const result5 = sentencesExercise5.filter((sentence) => {
...
const result6 = productsExercise6.filter((product) => product.quantity > 0);
console.log(result6);
...
const result7 = numbersExercise7.filter((number) => number % 3 === 0);
...
const result8 = wordsExercise8.filter((word) => word.length <= 5);
console.log(result8);
...
const result9 = studentsExercise9.filter(
  (student) => student.grade.startsWith('A') || student.grade.startsWith('B')
);
console.log(result9);
...
const result10 = usersExercise10.filter((user) => !user.name.startsWith('J'));
