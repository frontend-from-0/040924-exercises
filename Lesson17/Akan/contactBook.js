...
function partialNameSearch(partialName, contactList) {
  const results = contactList.filter(contact => contact.name.includes(partialName));
  if (results.length > 0) {
    results.forEach(contact => {
      console.log(`name: ${contact.name}, phone:${contact.phone}, email:${contact.email}`);
    });
  } else {
    console.log(`No contacts found with the partial name: ${partialName}`);
  }
}
...
function searchByField(value, contactList) {
  ...
  if (results.length > 0) {
    ...
    console.log(`No contacts found with the value: ${value}`);
  }
}

// Testing the optional enhancements
console.log('\nPartial name search for "li":');
partialNameSearch('li', contacts);

console.log('\nSorting contacts:');
sortContacts(contacts);
displayAllContacts(contacts);

console.log('\nSearch by field for "555":');
searchByField('555', contacts);
