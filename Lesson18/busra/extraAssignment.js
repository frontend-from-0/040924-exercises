...
class Restaurant {
  ...
  viewMenu() {
      ...
      this.menu.forEach((item) => {
          console.log(`${item.name} (${item.category}): ${item.price}TL`);
      });
  }
  ...
  placeOrder(order) {
      console.log(`Restoran: ${this.name}`);
...
class Order {
  ...
  addItem(name, quantity) {
      const menuItem = restaurant.getMenuItem(name);
      if (!menuItem) {
          console.log(`${name} adlı ürün menüde bulunamadı.`);
          return;
      }
  ...
  viewOrder() {
      let details = "Sipariş Detayları:\n";
      this.orderItems.forEach((item, index) => {
          details += `${index + 1}. Ürün: ${item.name}, Miktar: ${item.quantity}, Birim Fiyat: ${item.pricePerUnit}\n`;
      });

      details += `Toplam Fiyat: ${this.calculateTotal()}`;
      return details;
  }
}


const restaurant = new Restaurant("Losteria", "Göppingen");
restaurant.addMenuItem("Margarita", 35, "Pizza");
restaurant.addMenuItem("Tiramisu", 15, "Tatlı");
restaurant.addMenuItem("Coca-Cola", 10, "İçecek");
restaurant.viewMenu();

const order = new Order();
order.addItem("Margarita", 3);
order.addItem("Coca-Cola", 2);
